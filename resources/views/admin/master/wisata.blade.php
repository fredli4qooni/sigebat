<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <!-- Papan Judul Seksi (DESIGN.md 7.11: Judul Halaman Adalah Papan Berwarna Seksinga) -->
        <x-papan warna="cokelat" class="p-6">
            <h2 class="text-2xl font-bold font-papan text-putih">Kategori Objek Wisata</h2>
            <p class="text-putih/90 text-sm mt-1">
                Klasifikasi jenis objek wisata Kampung Gedung Batin (misalnya: Wisata Budaya, Wisata Alam, Agrowisata).
            </p>
        </x-papan>

        <!-- Form Tambah Kategori -->
        <div class="bg-putih border-2 border-aspal rounded-papan p-6">
            <h3 class="text-lg font-bold font-papan text-aspal mb-4">Tambah kategori baru</h3>
            <form method="POST" action="{{ route('admin.master.wisata.store') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                ...
                <div>
                    ...
                    <x-text-input id="nama" name="nama" type="text" :value="old('nama')" required placeholder="Contoh: Wisata Alam & Hutan" />
        ...
        <!-- Tabel Data Master (DESIGN.md 7.9) -->
        <div class="bg-putih border-2 border-aspal rounded-papan overflow-hidden">
            <div class="p-4 border-b border-beton flex items-center justify-between">
                <h3 class="font-bold font-papan text-aspal text-lg">Daftar kategori wisata</h3>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-aspal text-putih text-sm font-semibold">
                            ...
                            <th class="py-3 px-4 text-center">Objek Terkait</th>
                            ...
                            <tr class="h-[60px] hover:bg-beton/40" x-data="{ editing: false }">
                                ...
                                <td class="py-3 px-4 text-center">
                                    @if($kategori->objek_wisata_count > 0)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-tag bg-cokelat text-putih text-xs font-bold">
                                            {{ $kategori->objek_wisata_count }} wisata
                                        </span>
                                    @else
                                        <span class="text-xs text-abu">0 wisata</span>
                                    @endif
                                </td>
                                <td class="py-3 px-4 text-right whitespace-nowrap">
                                    <template x-if="!editing">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.master.wisata.destroy', $kategori) }}" onsubmit="return confirm('Hapus kategori \'{{ $kategori->nama }}\'?');" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" @if($kategori->objek_wisata_count > 0) disabled title="Tidak dapat dihapus karena masih digunakan" @endif class="px-3 py-1.5 bg-putih border-2 border-merah rounded-kontrol text-xs font-bold text-merah hover:bg-merah hover:text-putih cursor-pointer disabled:opacity-40 disabled:hover:bg-putih disabled:hover:text-merah disabled:cursor-not-allowed">
                                                    Hapus
                                                </button>
                                            </form>
                                        </div>
                                    ...
                                    <template x-if="editing">
                                        <div class="flex items-center justify-end gap-2">
                                            <button type="submit" form="edit-form-{{ $kategori->id }}" class="px-3 py-1.5 bg-aspal text-putih rounded-kontrol text-xs font-bold hover:bg-black cursor-pointer">
                                                Simpan
                                            </button>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-8 text-center text-abu">
                                    Belum ada kategori wisata. Tambahkan kategori pertama di atas.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
